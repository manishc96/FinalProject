...
router.post('/forgotpassword/:id', async (req, res) => {
    try {
        let user = await model.register.findOne({
        ...
        let comparepassword = await bcrypt.compare(req.body.userPassword, user.UserLogin.userPassword);
        console.log(comparepassword);
        if (comparepassword) { return res.status(401).send({ message: 'this is old password make a new password!' }) }
        let salt = await bcrypt.genSalt(10);
        user.UserLogin.userPassword = await bcrypt.hash(req.body.userPassword, salt);
